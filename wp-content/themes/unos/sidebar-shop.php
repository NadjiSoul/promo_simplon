		...
		elseif ( current_user_can( 'edit_theme_options' ) ) : // If the sidebar has no widgets.
			...
			the_widget(
				...
				array(
					...
					'text'   => sprintf( __( 'Woocommerce pages have a separate sidebar than the rest of your site. You can add custom widgets from the %1$swidgets screen%2$s in wp-admin.<br /><br />(This widget is only displayed to logged in admins when no widget has yet been added to this area.) <strong>Your visitors will not see this text.<strong>', 'unos' ), '<a href="' . esc_url( admin_url( 'widgets.php' ) ) . '">', '</a>' ),
					'filter' => true,
				),
				array(
