		...
		if ( is_customize_preview() ) {
			...
			foreach ( $logo_custom as $logo_custom_line ) {
				...
				if ( $lcount == 1 && !empty( $logo_custom_line['size'] ) ) {
					$css .= '.site-logo-custom .site-title i {line-height:' . esc_attr( $logo_custom_line['size'] ) . '}';
				}
				$lcount++;
			}
			$inlinecss = apply_filters( 'unos_customtext_selectiverefresh_inlinecss', true );
			if ( $inlinecss ) echo '<style type="text/css">' . $css . '</style>';
		}
